...
namespace TigerRAG.Infrastructure.Auth.Dal;
    ...
    IUnitOfWork unitOfWork) : IRefreshSessionDal
{
    ...
    /// <summary>根据未撤销的 refresh token 查找关联用户；token 不存在或已撤销返回 null。退出登录审计需要先拿到用户身份再撤会话。</summary>
    public async Task<UserAccount?> GetUserByTokenAsync(string value, CancellationToken cancellationToken)
    {
        var hash = Hash(value);
        var token = await dbContext.RefreshTokens
            .SingleOrDefaultAsync(item => item.TokenHash == hash && item.RevokedAt == null, cancellationToken);
