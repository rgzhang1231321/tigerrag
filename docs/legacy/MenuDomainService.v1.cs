// 历史快照：MenuDomainService 旧版本（v1）。
// 原位于 src/TigerRAG/Application/Security/MenuDomainService.cs，因 src/TigerRAG/
// 不在解决方案中、未被编译、未被任何代码引用，迁移至此仅作历史保留。
// 当前生产代码使用 src/TigerRAG.Application/Menus/MenuDomainService.cs（v2）。
// 差异：v1 使用旧 API（ValidatePermission 校验 SystemPermissions.All，已无该类型）；
//      v2 改为角色名单校验（ValidateRoles）和 BFS 向下后代循环检测。
