...
export function MenuManagement() {
  ...
  const roleOptions = useMemo(() => allRoles.map((r) => r.name), [allRoles]);
  ...
  const columns: ColumnsType<TreeMenuNode> = [
    { title: "Key", dataIndex: "key", key: "key", width: 160 },
    {
      ...
      render: (label: string, node) => {
        ...
        return (
          <Space size={6}>
            <Tag color={isRoot ? "default" : "default"}>
              {isRoot ? "顶级" : "子级"}
            </Tag>
            <span>{label}</span>
          </Space>
        ...
        <Form layout="vertical" className="users-form" initialValues={formData}>
          ...
          <Form.Item label="父菜单（空 = 顶级）">
            <Select
              ...
              onChange={(value) =>
                setFormData((prev) => ({ ...prev, parentId: value || null }))
              }
          ...
          <Form.Item label="排序号">
            <InputNumber
              ...
              onChange={(value) =>
                setFormData((prev) => ({ ...prev, sortOrder: value ?? 0 }))
              }
              style={{ width: "100%" }}
            />
          </Form.Item>
