...
import { useDocumentPermissions, useReplaceDocumentPermissions } from './useDocuments'
...
/// <summary>文档权限管理 Modal 属性。</summary>
interface DocumentPermissionModalProps {
  /// <summary>被授权文档 Id。</summary>
  documentId: string
  /// <summary>被授权文档文件名，用于标题显示。</summary>
  fileName: string
...
/// <summary>
/// 文档权限管理 Modal：双面板（用户/角色）+ 搜索 + checkbox + 已授权摘要。
/// 整体替换语义：保存时提交全部选中项，后端覆盖已有 ACL。
/// 降级保护：GET 当前权限失败时显示警告，checkbox 全部未勾选，用户仍可操作。
/// </summary>
export function DocumentPermissionModal({ documentId, fileName, open, onClose }: DocumentPermissionModalProps) {
  // 查询当前权限状态（降级保护）
  const {
    data: currentPermissions,
    isPending: permissionsLoading,
    isError: permissionsError,
  } = useDocumentPermissions(open ? documentId : null)
  ...
  // 替换权限 mutation
  const replaceMutation = useReplaceDocumentPermissions()
